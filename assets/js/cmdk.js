(() => {
    ...
    const render = () => {
        ...
        const html = Object.entries(groups).map(([g, arr]) => `
            ...
            ${arr.map(it => `
                <div class="cmdk-items${it.idx === active ? ' active' : ''}" data-idx="${it.idx}" role="option">
    ...
    const filter = (q) => {
        ...
        filtered = ql
            ? items.filter(i => (i.label + ' ' + (i.k||'') + ' ' + (i.hint||'')).toLowerCase().includes(ql))
            : items;
        active = 0;
        render();
    };
    ...
    const run = (it) => {
        if (!it) return;
        if (it.js) {
            try { (new Function(it.js))(); } catch (e) { console.warn(e); }
        }
        if (it.href) {
    ...
    const open = () => {
        if (typeof dialog.show === 'function') dialog.show(); else dialog.hidden = false;
        $q.value = '';
        filter('');
    ...
    $q.addEventListener('keydown', e => {
        if (e.key === 'ArrowDown')    { e.preventDefault(); active = Math.min(filtered.length - 1, active + 1); render(); ensureVisible(); }
